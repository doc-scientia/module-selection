...
def test_student_can_get_subscribed_modules(
        client, module_factory, enrolment_factory
):
    module = module_factory()
    enrolment_factory(module=module.id, student_username='hpotter')
    with client:
        res = client.get(f"/{module.year}/subscribed_modules/hpotter", auth=HPOTTER_CREDENTIALS)
    assert res.status_code == 200
    assert len(res.json()) == 1
    assert res.json()[0] == module.module_code


def test_student_can_select_valid_module_selection(
        client, module_factory, session
):
    modules = module_factory.create_batch(size=5)
    module_codes = [m.module_code for m in modules]

    with client:
        res = client.post(f"/{2223}/subscribed_modules/hpotter", auth=HPOTTER_CREDENTIALS,
    ...
    query = (
        select(
            Enrolment
        ).where(Enrolment.student_username == "hpotter")
    )
    new_enrolment_module_ids = [new_enrolment.module for new_enrolment in session.exec(query).all()]

    assert new_enrolment_module_ids == [module.id for module in modules]


def test_student_cannot_select_invalid_module_selection(
        client
):
    modules = []
    with client:
...
def test_all_modules_for_year_can_be_retrieved(
